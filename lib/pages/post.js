(function(){
  ...
  function getter(i){
    if(i<=total){
      document.querySelector(".page .post .seemore").style.display="none";
      if(!document.querySelector(".page .post .postlist .loading_fh")){
        document.querySelector(".page .post .postlist").innerHTML+='<div class="loading_fh"></div>';
      }
      get('./post_comment/'+i+'.json',function(res){
        document.querySelector(".page .post .postlist .loading_fh").remove();
        res=JSON.parse(res);
        total=res.total
        var str='';
        ...
        document.querySelector(".page .post .seemore").style.display="block";
      });
