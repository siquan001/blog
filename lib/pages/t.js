(function(){
  ...
  function getter(i){
    if(i<=total){
      document.querySelector(".page .t .seemore").style.display="none";
      document.querySelector(".page .t .t_list").innerHTML+='<div class="loading_fh"></div>';
      get('./t_comment/'+i+'.json',function(res){
        document.querySelector(".page .t .t_list .loading_fh").remove();
        res=JSON.parse(res);
        total=res.total
        var str='';
        res.data.forEach(function(item){
          str+='<li><a href="#/t/'+item.id+'">\
          <div class="desc">'+item.desc.replace('<','&lt;').replace('>','&gt;')+'</div>\
          <div class="time">'+new Date(item.time).toDateString()+'</div>\
          </a></li>';
        });
        document.querySelector(".page .t .t_list").innerHTML+=str;
        document.querySelector(".page .t .seemore").style.display="block";
      })
      if(i==total){
